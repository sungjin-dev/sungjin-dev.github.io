...
<div id="wrap"></div>
``` 
본문에서는 이와 같은 방식으로 코딩되었을 것이다. 
<br><br>
`div.head` 이 부분은 위에서 배운 클래스 선택자를 떠올려보면 된다. 
<br>
다른 부분은 앞에 어떤 태그인지까지 정확히 적고 그 다음 . 을 찍고 클래스명을 적었다.
<br><br>
```html
<div id="wrap"> 부모 안에
...
```
<br>
이런식으로 파고 들어가는 구조를 입력할 때는 어떤 태그의 클래스 명인지 명확하게 적어줘야한다. 
<br><br>
그리고 선택자 사이에 **띄어쓰기**가 있는데 `선택자 -> 선택자` 이렇게 해당 하위 태그로 파고 들어갈 수 있다.
<br>
(맛보기로 직계자손은 >, 그냥 후손들은 띄어쓰기로 안으로 들어가서 지정할 수 있다.)
<br><br>
```css
#wrap div.head {}
```
...
<br>
div태그를 달고 있으면서 클래스 명이 head인 이 녀석(?)을 선택해서 작업하고 싶을 때! 이렇게 입력하면 된다. 
 
<div class="blank-space"></div>
본문 html 구조를 보면 좀 더 직관적으로 이해하기 쉽다. 
```html
<div id="wrap">   <--- 아이디는 고유하며, 가장 바깥쪽에서 감싸고 있는 태그에 지정한다.          
    <div class="to">    
        <h1>
