...
<br><br>
원래는 웹 브라우저 안에서만 동작하는 언어였지만, **Node.js**라는 실행 환경이 등장하면서 서버(백엔드) 프로그램도 자바스크립트로 개발할 수 있게 되었다. 한마디로 프론트엔드와 백엔드를 모두 아우르는 **'풀스택(Full-stack)'** 개발이 가능해진 것! 그래서 이 자바스크립트의 인기가 치솟게 된 것이다.
<br>
