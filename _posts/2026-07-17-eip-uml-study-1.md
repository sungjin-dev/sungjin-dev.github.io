...
> 💡 **암기 포인트**: "명세화·시각화·문서화" 세 단어는 서술형으로 그대로 물어본다.
...
> 💡 **암기 포인트**: "사·관·다" — 명사/동사(사물), 형용사/부사(관계) 대응까지 세트로 외운다.
...
> 💡 **포함 vs 확장 구분법**: 포함은 "반드시 실행"(필수), 확장은 "실행할 수도 있음"(선택). 화살표 방향이 서로 반대인 것도 자주 출제된다.
...
> 💡 **포함 vs 집합**: 엔진-피스톤(포함, 생명주기 공유)과 차-엔진(집합, 독립적 존재 가능)의 예시로 구분한다. 마름모가 채워졌는지(포함)/비었는지(집합)로 표기를 구분한다.
...
> 💡 "길러멧"이라는 기호 명칭 자체가 단답형으로 출제된 적 있다.
