# 파이썬 re 모듈 4대장 완벽 비교 — match, search, findall, sub 한 번에 정리

지난 글에서 `re.match()`를 다루면서 "문자열의 맨 앞이 패턴과 일치하는지" 검사하는 법을 배웠습니다. 그리고 마지막에 약속드렸죠. 이번 글에서는 `re.search()`, `re.findall()`, `re.sub()`까지 묶어서 **re 모듈 4대장**을 한 번에 비교해 보겠습니다.
...
| `re.findall()` | 문자열 전체에서 **일치하는 모든 것**을 수집 | 리스트 |
...
## 1. re.search() — "어딘가에 있는가?"를 찾는 탐색기

`re.match()`가 문지기라면, `re.search()`는 수색대입니다. 문자열의 처음부터 끝까지 훑으면서 **패턴과 일치하는 첫 번째 지점**을 찾아냅니다.
...
같은 패턴, 같은 문자열인데 결과가 다르죠? `match()`는 맨 앞만 보기 때문에 실패했지만, `search()`는 문장 중간에 있는 `"SungJin"`을 찾아냈습니다.

### 실전 예시 — 로그에서 특정 사용자 활동 찾기

서버 로그 한 줄에서 사용자 이름과 행동을 뽑아내는 상황을 생각해 볼게요.
...
지난 글에서 배운 그룹 `( )` 문법이 그대로 쓰였습니다. `search()`도 Match 객체를 돌려주기 때문에 `.group()`, `.group(1)` 같은 메서드를 똑같이 사용할 수 있고, **실패하면 None을 반환하므로 `if m:` 확인도 똑같이 필요**합니다. match에서 배운 습관이 그대로 통하는 셈이죠.
...
단, `search()`도 **첫 번째 하나만** 찾고 멈춘다는 점을 기억하세요. "전부 다" 찾고 싶다면? 다음 주자가 나설 차례입니다.

---

## 2. re.findall() — "전부 다 찾아와"라고 시키는 수집기

`re.findall()`은 이름 그대로 **일치하는 모든 부분을 찾아서 리스트로** 돌려줍니다. Match 객체가 아니라 **문자열 리스트**를 반환한다는 게 앞의 두 함수와 다른 점입니다.
...
일치하는 게 하나도 없으면 에러가 아니라 **빈 리스트 `[]`**가 나옵니다. 그래서 findall의 결과는 보통 for문이나 `len()`과 함께 씁니다.
...
입문자분들이 findall에서 가장 많이 당황하는 지점입니다. 패턴에 그룹 `( )`이 있으면, findall은 **전체 매치가 아니라 그룹 안의 내용만** 모아 옵니다.
...
그룹이 2개 이상이면 튜플의 리스트가 됩니다. "왜 내 결과가 이상하게 나오지?" 싶을 때는 패턴 속 괄호부터 의심해 보세요.

---

## 3. re.sub() — 찾아서 바꾸는 치환기

`re.sub(패턴, 바꿀_내용, 문자열)`은 패턴과 일치하는 부분을 **다른 문자열로 바꾼 새 문자열**을 돌려줍니다. 에디터의 "찾아 바꾸기"의 정규표현식 버전이라고 생각하시면 됩니다.
...
개인정보 마스킹, 공백 정리, 금지어 필터링처럼 실무에서 정말 자주 쓰입니다.

### 그룹 재활용 — \1, \2로 잡은 내용 다시 쓰기

sub의 진짜 매력은 **매치된 내용을 치환문에서 재사용**할 수 있다는 점입니다. 바꿀 내용 안에서 `\1`, `\2`는 각각 첫 번째, 두 번째 그룹을 가리킵니다.
...
치환문에도 `\1`처럼 백슬래시가 들어가므로, 패턴뿐 아니라 **치환 문자열에도 `r"..."`을 붙이는 습관**을 들이세요.
...
예를 들어 `"질문 있으면 sungjin@example.com 으로 메일 주세요!"`라는 댓글을 등록하면, 화면에는 `"질문 있으면 [이메일 비공개] 으로 메일 주세요! (가려진 이메일: 1개)"`가 표시됩니다. findall로 개수를 세고 sub로 치환하는, 4대장 중 두 명의 협업 플레이입니다.

---

## 4. 상황별 선택 가이드 — 이럴 땐 이 함수

머릿속에 흐름도 하나만 넣어두면 됩니다.
...
같은 문자열에 4대장을 전부 투입해 보면 차이가 한눈에 보입니다.
...
print(results.group())   # 에러! findall은 리스트를 반환합니다
```

Match 객체를 돌려주는 건 `match()`와 `search()`뿐입니다. `findall()`의 결과는 평범한 리스트이므로 인덱싱(`results[0]`)이나 for문으로 다루세요.
...
`re.sub()`은 원본을 수정하지 않고 **새 문자열을 반환**합니다. 결과를 쓰려면 반드시 변수에 담으세요. (`text = re.sub(...)`)

---

## 마무리

오늘 배운 4대장을 다시 한 줄로 정리합니다.
...
이 네 개만 자유롭게 다뤄도 실무 정규표현식의 8할은 해결됩니다. 다음 글에서는 같은 패턴을 여러 번 쓸 때 성능을 높여주는 `re.compile()`과, 패턴을 훨씬 유연하게 만들어주는 플래그(`re.IGNORECASE` 등)를 다뤄보겠습니다. 궁금한 점은 댓글로 남겨주세요!
