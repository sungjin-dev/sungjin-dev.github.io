...
```
참고 : `\w (word)`: 구체적으로는 영어 대소문자(a-z, A-Z), 숫자(0-9), 언더스코어(_)가 여기에 해당. [a-zA-Z0-9_]와 동일하다. 
`+`는 "앞의 것이 1개 이상 반복" 
<br>
이제 패턴 전체를 로그에 대입해 보면,

```python
user=(\w+) action=(\w+)
user=SungJin action=login
     └──┬──┘        └─┬─┘
     그룹 1         그룹 2
```

**user=** : 로그에서 글자 그대로 user=를 찾음 <br>
**(\w+)** : 그 뒤에 이어지는 단어 덩어리 → SungJin, 첫 번째 괄호이므로 '그룹 1' <br>
 **action=** : 공백 포함해서 글자 그대로  action= <br>
**(\w+)** : 또 단어 덩어리 → login, 그룹 2 <br>

그래서 m.group(1)이 "SungJin", m.group(2)가 "login"을 돌려주게 된다.  <br>
여기서 만약 (\w)처럼 +가 없다면 **"단어 문자 딱 1개"** 만 매치돼서 그룹 1에 `S` 한 글자만 나온다. + 덕분에 SungJin 전체가 나오게 된다.  <br>
 <br><br>
지난 글에서 배운 그룹 `( )` 문법이 그대로 쓰인 것을 알 수 있다. `search()`도 Match 객체를 돌려주기 때문에 `.group()`, `.group(1)` 같은 메서드를 똑같이 사용할 수 있으며, <br> 
**실패하면 None을 반환하므로 `if m:` 확인도 똑같이 필요** 하다.

### match vs search, 언제 뭘 쓸까?

- **형식 검증** (아이디, 전화번호, 날짜가 규칙에 맞는가?) → `match()` <br>
- **포함 탐색** (이 텍스트 어딘가에 원하는 게 있는가?) → `search()` <br>
<br>
...
예를 들어 `"질문 있으면 sungjin@example.com 으로 메일 주세요!"`라는 댓글을 등록하면, 화면에는 `"질문 있으면 [이메일 비공개] 으로 메일 주세요! (가려진 이메일: 1개)"`가 표시된다. findall로 개수를 세고 sub로 치환하는 것이다.
