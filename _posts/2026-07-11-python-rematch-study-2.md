...
지난 글에서 `re.match()`를 다루면서 "문자열의 맨 앞이 패턴과 일치하는지" 검사하는 법을 배웠보았다. 이번 글에서는 `re.search()`, `re.findall()`, `re.sub()`까지 묶어서 **re 모듈 4대장**을 한 번에 비교해 보자.
...
같은 패턴, 같은 문자열인데 결과가 다르다. `match()`는 맨 앞만 보기 때문에 실패했지만, `search()`는 문장 중간에 있는 `"SungJin"`을 찾아낸다.
