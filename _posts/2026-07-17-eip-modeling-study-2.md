...
> ![star] **암기 포인트**: 구조적 = DFD·DD / 객체 지향 = UML. 방법론과 표기법 짝짓기가 출제된다.
...
> ![star] CASE 주요 기능 5가지는 기출 횟수가 가장 많은 항목 중 하나다. "그래픽·전 단계 연결·다양한 모형·표준화/문서 자동화·커뮤니케이션"으로 통째로 외운다.
...
> ![star] **구분법**: CLI(텍스트) → GUI(그래픽) → NUI(신체·음성) → OUI(사물 자체). 발전 순서대로 외우면 각 유형의 키워드가 자연스럽게 이어진다.
...
> ![star] **암기 포인트**: "**직·유·학·유**" 4대 원칙은 최상위 빈출. 특히 각 원칙과 설명을 매칭하는 문제가 자주 나오니 키워드(직관성=쉽게 이해, 유효성=목표 달성, 학습성=배우고, 유연성=실수 방지)를 정확히 구분한다.
...
> ![star] 라디오 버튼(1개만) vs 체크 박스(1개 이상) 구분이 함정으로 나온다.
...
> ![star] **구분 함정**: 프로토타입 = **동적**(시뮬레이션 가능) / 목업 = **정적**(구현 안 됨). "동적 효과"와 "실제로 구현되지 않음"이 각각의 결정적 키워드다.
