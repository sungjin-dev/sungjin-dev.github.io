...
YOLO로 학습을 진행하면 'my_experiment_v1' 폴더에 다음과 같은 결과물들이 나온다. 
...
나중에 직접 해보면 알겠지만 `val_batch0_pred.jpg`(학습 중인 모델이 검증 -Validation- 이미지에 직접 바운딩 박스를 친 결과물)이 엄청 중요하다. 학습을 돌려놓으면 처음 1에포크가 끝나자마자 바로 저 사진이 폴더에 생성되는데 그걸 보면서 클래스가 잘 설정되었는지 라벨링이나 데이터셋 구성은 잘 됐는지 학습은 잘 하고 있는지 등을 초기에 빠르게 판단할 수 있다. 

처음 학습할 때에는 이런 노하우도 없어서 학습이 다 종료되고 나서 피드백했었는데 val_batch0_pred.jpg의 존재를 미리 알아두면 좋다. results.csv도 좋긴한데 실수로 수정하게 되면 학습이 종료돼서 last.py파일을 이용해서 학습을 재개해야 한다. 이후에 이 부분에 대해서는 자세히 설명하겠다. 
