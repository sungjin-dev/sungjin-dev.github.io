...
물론 `results.csv`도 초기에 생성되며 학습 기록이 상세히 적힌다. 저위 그래프들도 사실은 results.csv 파일 안의 숫자들을 읽어서 선으로 이어 그린 것에 불과하다. 하지만 실수로 수정하게 되면 학습이 종료돼서 last.pt파일을 이용해서 학습을 재개해야 한다. 그리고 사진으로 보는 것이 보다 직관적으로 파악하기 좋다. results.csv는 보다 정밀분석이 필요한 경우 초기에 살펴보자. 이후에 이 부분에 대해서는 자세히 설명하겠다. 
...
위에서 설명했듯이 이렇게 낮은 형태의 F1곡선과 AP면적이 홀쭉한 클래스의 경우 학습이 잘 되지 않았다는 증거다. 그래서 눈물을 머금고 과감하게 클래스에서 제외시킨 후에 다시 학습을 재개했다. 학습효과가 1.5배 정도 향상되었는데  자세한 사연은 후속글에 남기겠다. 
