...
> **🧠 파국적 망각(Catastrophic Forgetting) 발생 흐름**
> 
> 1️⃣ **기존 모델 학습** 
>   `[사람, 자동차, 개]`를 인식하도록 전체 가중치(Weight) 조정 완료
> ⬇️
> 2️⃣ **새 클래스 추가 학습** 
>   기존 데이터 없이 `[고양이]` 데이터만 넣고 이어서 학습(Fine-tuning) 진행
> ⬇️
> 3️⃣ **가중치 업데이트 편향** 
>   모델의 가중치가 오직 "고양이 인식"에만 맞게 집중적으로 업데이트됨
> ⬇️
> 💥 **기존 기억 덮어쓰기 (Overwrite)** 
>   "사람, 자동차, 개"를 인식하던 기존 가중치 패턴이 훼손되고 지워져버림던 가중치 패턴이 점점 덮어써짐(overwrite)
