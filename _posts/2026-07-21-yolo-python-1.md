...
처음 학습할 때에는 이런 노하우도 없어 학습이 다 종료되고 나서 피드백을 하곤 했다. 경험상 val_batch0_pred.jpg의 존재를 미리 알아두면 좋다. `results.csv`도 초기에 생성되며 학습 기록이 상세히 적힌다. 저위 그래프들도 사실은 results.csv 파일 안의 숫자들을 읽어서 선으로 이어 그린 것에 불과하다. 하지만 실수로 수정하게 되면 학습이 종료돼서 last.pt파일을 이용해서 학습을 재개해야 한다. 그리고 사진으로 보는 것이 보다 직관적으로 파악하기 좋다. results.csv는 보다 정밀분석이 필요한 경우 초기에 살펴보자. 이후에 이 부분에 대해서는 자세히 설명하겠다. 
