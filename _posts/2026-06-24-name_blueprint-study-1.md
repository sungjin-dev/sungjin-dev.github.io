...
<br><br>
그런데 파일이 점점 커지기 시작한다면 이야기가 달라진다. 
...
`main.py`파일에 import sign_up 와 같은 방식으로 연동시켜서 실행하면 
 ...
 파이썬은 해당 import된 파일 sign_up를 처음부터 끝까지 전부 읽어버린다.
...
<br>
그래서 나온 것이 `if __name__ == "__main__": app.run()` 이다.

## 5. name 의 정체

파이썬은 파일을 실행할 때 자동으로 `__name__` 이라는 변수를 만들어준다.

즉 직접 실행하면 python app.py 이면 `__name__ == "__main__"` 이다.
...
그런데 import형식으로 끌어오는 경우라면 상황이 다르다. 
...
이를 이용해서 각 모듈만 실행시킬 수도 있고 다양한 방식으로 활용가능하다. 

정리하면 if __name__ == "__main__": 는 프로그램의 시작점을 구분하는 장치다.

즉 `직접 실행`과 `import`를 구분한다. 그렇게 `실행 경계` 를 만든다.

## 6. Flask는 왜 모듈화를 하게 될까?
...
그래서 Flask의 성장 과정은 다음과 같이 정리 가능하다. 
