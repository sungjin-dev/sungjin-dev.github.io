...
* **M (Model) : 주방장과 식재료 창고**
  ...
  * **비유:** 식당의 주방장. 창고에서 고기를 꺼내오고 레시피에 따라 요리를 만든다. Model은 데이터를 저장하고 조회하는 기능뿐 아니라, 데이터를 처리하는 비즈니스 규칙까지 담당한다.
<br><br>
* **V (View) : 플레이팅과 손님 테이블**
...
## 2. Flask 코드에 대입해 보면?
<br><br>
Flask 프로젝트는 MVC 패턴을 참고하여 구조화하는 경우가 많다. 

```python
# Controller (웨이터)
@app.route('/profile/<username>') (참고로 Blueprint를 활용하여 다수 활용될 수 있다)
...
<br><br>
서로의 영역을 침범하지 않으니, 프로그램이 훨씬 거대해져도 유지보수가 편해지고 역할이 명확하게 분리되어 협업과 테스트가 편해진다. M.V.C는 꼭 명심하자!.
<br><br>
<심화내용>
<br><br>
현재 Flask를 MVC보다 MVT에 가깝다고 보는 시각도 존재한다. 
...
View
```
인 `MVT`로 해석하는 시각이 있는데 `Django`가 이를 공식적으로 칭한 바 있다. (참고로 Django(장고)는 파이썬(Python)으로 만들어진 각광받는 오픈소스 웹 프레임워크다)
<br>
웹 프레임워크마다 MVC를 조금씩 다르게 해석할 수 있다는 점도 알아두도록 하자. 
