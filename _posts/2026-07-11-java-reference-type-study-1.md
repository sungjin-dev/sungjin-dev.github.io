...
참고로 『이것이 자바다』(2021)를 베이스로 공부하며 정리한 글이라, 지금 기준(Java 17~21 시대)으로 보충이 필요한 부분은 중간중간 따로 짚었다.
...
```plaintext
┌────────────────────────────────────────────────────────────────────────────────┐
│  [ JVM 실행 메모리 영역 (Runtime Data Area) ]                                  │
│                                                                                │
│ ┌─────────────────────────────────────┐  ┌───────────────────────────────────┐ │
│ │ 1. JVM 스택 (Stack)                 │  │ 2. 힙 영역 (Heap)                 │ │
│ │    - 스레드마다 독립 생성           │  │    - 모든 스레드가 공유           │ │
│ │                                     │  │    - GC의 메인 무대               │ │
│ │ ┌─────────────────────────────────┐ │  │                                   │ │
│ │ │ [스레드-1]                      │ │  │  객체-1  배열-2  객체-n ...       │ │
│ │ │                                 │ │  │                                   │ │
│ │ │ ┌─────────────────────────────┐ │ │  │ ┌───────────────────────────────┐ │ │
│ │ │ │ 프레임-n (최신 메서드)      │ │ │  │ │ [NEW] static 변수             │ │ │
│ │ │ │  - 지역변수 (값/번지 저장)  │ │ │  │ │  (java.lang.Class 객체 소속)  │ │ │
│ │ │ └─────────────────────────────┘ │ │  │ └───────────────────────────────┘ │ │
│ │ │      |                          │ │  │ ┌───────────────────────────────┐ │ │
│ │ │      |  push / pop              │ │  │ │ [NEW] 런타임 상수풀           │ │ │
│ │ │      v                          │ │  │ │  (문자열 리터럴 등)           │ │ │
│ │ │ ┌─────────────────────────────┐ │ │  │ └───────────────────────────────┘ │ │
│ │ │ │ 프레임-1 (시작 메서드)      │ │ │  │                                   │ │
│ │ │ │  - 지역변수 (값/번지 저장)  │ │ │  │  * 원래 메서드 영역 소속,         │ │
│ │ │ └─────────────────────────────┘ │ │  │    Java 8+ 부터 힙에서 관리       │ │
│ │ └─────────────────────────────────┘ │  └───────────────────────────────────┘ │
│ │                                     │                                        │
│ │  [스레드-2] ... [스레드-n]          │                                        │
...
└────────────────────────────────────────────────────────────────────────────────┘
                               ...
                               (클래스 설계도 참조)
                                         ...
                                         v
┌────────────────────────────────────────────────────────────────────────────────┐
│ 3. 네이티브 메모리 (Native Memory) - OS가 직접 관리                            │
│                                                                                │
│  ┌──────────────────────────────────────────────────────────────────────────┐  │
│  │ [NEW] 메타스페이스 (Metaspace)                                           │  │
│  │  - 기존 "메서드 영역"의 후신                                             │  │
│  │  - static을 제외한 순수 설계도(클래스 구조)만 보관                       │  │
│  │                                                                          │  │
│  │  ┌────────────────────────┐   ┌────────────────────────┐                 │  │
│  │  │ 클래스-1 구조 정보     │   │ 클래스-n 구조 정보     │                 │  │
│  │  │  - 바이트코드 (Code)   │   │  - 바이트코드 (Code)   │                 │  │
│  │  │  - 메서드/생성자 정보  │   │  - 메서드/생성자 정보  │                 │  │
│  │  └────────────────────────┘   └────────────────────────┘                 │  │
│  └──────────────────────────────────────────────────────────────────────────┘  │
└────────────────────────────────────────────────────────────────────────────────┘
```

### 1. JVM 스택 (Stack): "철저한 보존 구역"
구조:스레드가 생성될 때마다 독립된 스택 줄기가 뻗어 나오며, 메서드 실행에 맞춰 프레임이 위로 쌓이고(push) 아래로 터지는(pop) 구조.

알맹이: 오직 메서드 내부의 임시 지역 변수들만 보관.

### 2. 힙 영역 (Heap Area): "대재개발 구역 (엄청 뚱뚱해짐)"
과거: new로 만든 일반 인스턴스 객체와 배열만 살던 동네.

현재:

메서드 영역에 살던 static 필드(변수)들이 전부 힙 영역으로 이사옴. 힙 영역 내부에 떠 있는 java.lang.Class 객체의 뱃속에 기생.

글자 데이터들을 모아두던 '런타임 상수풀(특히 문자열 풀)'도 힙 영역으로 편입.

이유: 프로그램이 끝날 때까지 안 지워지던 덩치 큰 녀석들을 힙으로 넘겨서, 가비지 컬렉터(GC)가 주기적으로 청소하여 메모리 부족을 방지하기 위함.

### 3. 메타스페이스 (Metaspace): "JVM 외부로의 독립"
과거 (책의 '메소드 영역'): JVM 내부의 PermGen이라는 제한된 방을 썼습니다. 데이터와 설계도가 엉켜있어 방이 쉽게 터짐.

현재: PermGen이라는 방을 완전히 부수고 JVM 외부인 컴퓨터의 실제 RAM(네이티브 메모리) 영역에 '메타스페이스'라는 새 건물을 지음.
