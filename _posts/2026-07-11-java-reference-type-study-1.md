...
자바의 데이터 타입은 기본 타입과 참조 타입 둘로 나뉜다. 기본 타입 8개는 금방 익히지만, 진짜 자바다운 코드는 전부 참조 타입 위에서 벌어진다. 이 글에서는 참조 타입이라는 주제를 처음부터 끝까지 한 번에 정리한다. 메모리 구조, 참조 변수의 비교, null, String, 배열, 열거 타입 순서로 간다. 분량이 있지만, 이 글 하나로 참조 타입의 큰 그림이 완성되도록 구성했다.
...
```plaintext
┌─────────────────────────────────────────────────────────────────────────────┐
│ ▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒ [ JVM Runtime Data Area (Managed by JVM) ] ▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒ │
└─────────────────────────────────────────────────────────────────────────────┘
  │
  ├─► [ 1. JVM 스택 영역 (Stack) ] ─── 스레드별 개별 독립 공간 (Private)
  │    │
  │    ├─── Thread-1 Stack ───►  [ 프레임-n (최신 메서드) ] ── 지역변수 (값/주소)
  │    │                              │  ▲
  │    │                              │  │  push / pop (실행 흐름)
  │    │                              ▼  │
  │    │                         [ 프레임-1 (시작 메서드) ] ── 지역변수 (값/주소)
  │    │
  │    └─── Thread-2 ~ n Stack (위와 동일한 구조로 각자 수행)
  │
  │
  └─► [ 2. 힙 영역 (Heap Area) ] ─── 모든 스레드 공유 & 가비지 컬렉터(GC) 관리
       │
       ├─── 일반 데이터 ──────►  [ 인스턴스 객체 ]  [ 배열 데이터 ]  ...
       │
       ├───  [이동] static  ──►  [ java.lang.Class 객체 ] 내부 기생 (Static 필드)
       │
       └───  [이동] 상수풀  ──►  [ 런타임 문자열 상수풀 (String Pool) ]
                                 * 원래 메서드 영역이었으나 Java 8부터 힙으로 편입


┌─────────────────────────────────────────────────────────────────────────────┐
│ ▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒ [ Native Memory (Managed by Operating System) ] ▒▒▒▒▒▒▒▒▒▒▒ │
└─────────────────────────────────────────────────────────────────────────────┘
  │
  └─►  [ 3. 메타스페이스 (Metaspace Area) ] ─── 기존 "메서드 영역"의 후신
       │
       │   * 힙 영역에 속한 static 변수들과 연결되어 클래스 설계도 제공
       │
       ├─── [ 클래스-1 구조 정보 ] ─── 바이트코드(Code) / 메서드 및 생성자 데이터
       │
       └─── [ 클래스-n 구조 정보 ] ─── 바이트코드(Code) / 메서드 및 생성자 데이터
```
<img width="1768" height="1695" alt="JVM_메모리구조" src="https://github.com/user-attachments/assets/05389f17-afcc-4b83-880f-ecf9146ce8e4" />
...
과거: new로 만든 일반 인스턴스 객체와 배열만 살던 .

현재:

메서드 영역에 살던 `static 필드(변수)`들이 전부 힙 영역으로 이사. 힙 영역 내부에 떠 있는 java.lang.Class 객체의 뱃속에 기생.

글자 데이터들을 모아두던 '런타임 상수풀(특히 문자열 풀 String Pool)'도 힙 영역으로 편입.

이유: 프로그램이 끝날 때까지 안 지워지던 덩치 큰 녀석들을 힙으로 넘겨서, 가비지 컬렉터(GC)가 주기적으로 청소하여 메모리 부족을 방지하기 위함.

### 3. 메타스페이스 (Metaspace): "JVM 외부로의 독립 - 구 메서드 영역"

이제 데이터(static 값)는 힙으로 쫓아내고, 오직 "클래스의 뼈대(바이트코드), 메서드의 구조" 같은 순수한 설계도 정보만 저장

과거 (책의 '메소드 영역'): JVM 내부의 PermGen이라는 제한된 공간에서 데이터와 설계도가 엉켜있어 방이 쉽게 터짐.

현재: `PermGen`이라는 방을 완전히 부수고 JVM 외부인 컴퓨터의 실제 RAM(네이티브 메모리) 영역에 `'메타스페이스'`라는 새 건물을 지음.

알맹이: 여기에는 값(Value)이나 변수가 전혀 살지 않는다. 오직 JVM이 코드를 실행할 때 읽어야 하는 "클래스 자체의 뼈대, 메서드 코드 자체, 생성자의 기계어 코드" 같은 순수 설계도(메타데이터)만 저장. RAM이 허용하는 한 용량이 자동으로 늘어나므로 메모리가 터질 일이 거의 없다.
ㅊ
