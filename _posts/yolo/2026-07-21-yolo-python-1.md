...
YOLO가 수백 번(Epochs) 반복하며 스스로 똑똑해지는 내부 논리는 사실 학생이 공부하는 과정과 완전히 똑같다. 우리도 시험 준비할 때 회독이 중요하다고 하듯 말이다. 
...
## 5. 기본적인 튜닝할 값들 (Hyperparameters) 소개

하이퍼파라미터 튜닝은 모델을 '어떻게' 학습시킬지 가이드라인을 정하는 것이다. 마치 디제잉(DJing)하는 것 같다. 가장 기본적이면서도 중요한 설정값들을 살펴보지.

**1) 학습 흐름을 제어하는 필수 설정**

`에포크 (Epochs)`: 전체 데이터셋을 처음부터 끝까지 한 번 다 훑어보는 것

너무 적게 설정하면 모델이 채 배우기도 전에 학습이 끝나버리고(언더피팅), 너무 많이 설정하면 정답을 아예 통째로 외워버려 새로운 사진을 줬을 때 멍청해지는 부작용(오버피팅)이 발생합니다. 보통 100~300 사이로 여유 있게 설정.

`인내심 (Patience / Early Stopping)`: 검증 성능이 특정 에포크 동안 개선되지 않으면 학습을 조기에 종료하는 기준값.

에포크를 300으로 넉넉하게 설정했더라도, 무식하게 끝까지 학습하는 것은 시간 낭비다.

Patience를 '50'으로 설정하면, 50번의 에포크를 도는 동안 모델의 성능(Loss나 mAP)이 전혀 개선되지 않을 때 "더 이상 발전이 없으니 여기서 조기 종료(Early Stopping)되며 스스로 학습을 멈춘다. 과적합을 막아주는 아주 스마트한 브레이크인 셈.

`배치 사이즈 (Batch Size)`: GPU 메모리에 한 번에 올려서 처리할 이미지의 개수(예: 8, 16, 32)

배치 사이즈가 너무 작으면 비행기가 난기류를 만난 것처럼 학습 방향이 요동치고, 너무 크면 컴퓨터 메모리가 터져버린다(OOM 에러). 본인의 그래픽카드(VRAM) 스펙이 허락하는 한 가장 크게(보통 16이나 32) 잡는 것이 안정적임.

**2) 데이터를 다채롭게 뻥튀기하는 마법, 데이터 증강 (Augmentation)**

똑같은 사진만 계속 보여주면 모델은 지루해하고 편향에 빠진다. 그래서 사진을 비틀고, 찌그러뜨리고, 색감을 바꿔서 '새로운 사진을 보는 것 같은 착각'을 일으키게 하는데, 이를 데이터 증강이라고 한다.

`모자이크 (Mosaic)`: 단순히 사진 한 장을 비트는 수준이 아니라, 서로 다른 4장의 사진을 무작위로 잘라내어 하나의 십자가 모양 패치워크로 합쳐버리는 기법이다.

앞서 언급했던 '문맥적 편향(Contextual Bias), 과적합(Overfitting)'을 억제하는 데 탁월하다. 항상 바다에만 있던 구명조끼가 모자이크를 통해 산이나 도심 배경의 조각과 강제로 섞이면서, 모델이 배경에 속지 않고 객체 자체의 생김새에 집중하게 된다.

또한 사진이 작게 축소되어 들어가므로 작은 객체(Small Object) 탐지 능력을 기하급수적으로 끌어올려 준다. 

물론 이론상으로는 다 그럴싸하고 좋아보이지만 실제 현실적인 한계도 있다. ESP32-CAM이라는 저가형 카메라 모듈로는 고화질로 실시간 영상을 수신해서 원할하게 객체 탐지하는 것은 거의 불가능하다. 프로젝트 때는 640x480의 해상도로 진행했는데 상당히 화질이 낮은 편이다. 보통 객체 탐지할 대상이 원래도 작은데 모자이크 4분할 속에 들어가면, 원래 크기의 1/4(면적 기준 1/16) 수준으로 줄어든다. 1280 해상도에서도 이 정도 배율로 줄어든 객체는 픽셀 정보가 너무 적어 모델이 특징(Feature)을 추출하기 어렵다. 기존 640에서 1280로 상향하고, 그 부담을 낮추기 위해 Batch Size을 16에서 8로 조정한 후 돌렸으나 중간에 터미널에 뜨는 Loss정보와 mAP정보를 보자마자 바로 학습을 종료했던 기억이 있다. 

**3) 추가 기능들**

**ⓐ기하학적 증강 (Geometric Augmentation)**

`Flip` (뒤집기): 좌우를 반전. (글자를 인식해야 하는 OCR 프로젝트 등에서는 끄는 것이 좋다.)

`Scale & Translation` (크기 조절 및 이동): 객체를 확대/축소하거나 도화지 밖으로 살짝 밀어내어 일부가 잘리게 만든다. 현실에서 객체의 일부가 가려지는 현상(Occlusion)을 시뮬레이션하여 모델의 강건성(Robustness)을 키워줌.

**ⓑ광학적 증강 (Photometric Augmentation)**

`HSV (Hue, Saturation, Value)`: 이미지의 색상, 채도, 명도를 무작위로 바꾼다. 아침, 점심, 저녁, 흐린 날씨 등 다양한 조명 환경에서 찍힌 것처럼 데이터를 뻥튀기하여 빛에 대한 모델의 적응력을 높여줌.


첨언을 해보자면, 튜닝을 할 때는 한 번에 여러 하이퍼파라미터를 동시에 변경하지 않는 것이 좋다. 

이는 실험군과 대조군을 비교하는 과학적 실험 방식과 같은 원리이다. **기존 모델을 대조군(Control Group)** 으로 두고, **하나의 하이퍼파라미터만 변경한 모델을 실험군(Experimental Group)** 으로 만들어 비교해야 성능 변화의 원인을 명확하게 파악할 수 있다. 만약 여러 값을 한꺼번에 변경하면 성능이 향상되거나 저하되었을 때 어떤 변수의 영향으로 결과가 달라졌는지 인과관계를 정확하게 판단하기 어렵다. 따라서 한 번에 하나의 요소만 변경하며 실험을 진행하는 것이 객관적이고 신뢰할 수 있는 튜닝 방법이다.
...
---
<br><br> 
## 7. Catastrophic Forgetting (치명적 망각)의 위협
...
[Step 1] 기존 학습 완료
- [사람, 자동차, 개]를 인식하도록 가중치 조정됨
          ↓
[Step 2] 새 클래스 추가 학습
- [고양이] 데이터만 입력하여 계속 학습 진행
          ↓
[Step 3] 가중치 편향 업데이트
- 가중치가 "고양이" 인식에만 맞게 새롭게 업데이트됨
          ↓
[Step 4] 기존 패턴 Overwrite (파국적 망각)
- 기존 "사람, 자동차, 개"를 인식하던 가중치 패턴이 완전히 덮어써짐
...
## 8. YOLO 학습을 하면서 느낀점(고충 및 깨달음)

**1) batch 사진과 results.csv 확인하기**

그 첫번째 회독이 끝나고 나오는 `val_batch0_pred.jpg`(학습 중인 모델이 검증 -Validation- 이미지에 직접 바운딩 박스를 친 결과물)이 생각보다 엄청 중요하다. 
...
처음 학습할 때에는 이런 노하우도 없어 학습이 다 종료되고 나서 피드백을 하곤 했다. 경험상 val_batch0_pred.jpg의 존재를 미리 알아두면 좋다. 

물론 `results.csv`도 초기에 생성되며 학습 기록이 상세히 적힌다. 저 위 그래프들도 사실은 results.csv 파일 안의 숫자들을 읽어서 선으로 이어 그린 것에 불과하다. 하지만 실수로 수정하게 되면 학습이 종료돼서 last.pt파일을 이용해서 학습을 재개해야 한다. 

그리고 batch사진으로 보는 것이 엑셀 데이터 보다 직관적으로 파악하기 좋다. results.csv는 보다 정밀분석이 필요한 경우 초기에 살펴보자. 

**2) 사전 데이터셋은 철저히 계확해서 꾸리기**

순수한 사진(raw0), 라벨링, 클래스 선정 등 데이터셋을 꾸리는 것이 거의 70% 이상 아니 그 이상 훨씬 더 중요할 수도 있다. 다양한 값들을 튜닝하고 분석하는 것은 이해하면 크게 어렵지 않으나 기초데이터셋을 꾸리는 것이 정말 중요하다는 것을 깨달았다. 
<br>
특히 제외했을 때 아쉬울 가능성이 가장 큰 클래스 하나 정도는 미리 라벨링해두는 편이 낫다고 본다. 학습 도중 불필요하다고 판단되면 클래스를 제거하는 건 어렵지 않지만, 반대로 나중에 추가하려면 데이터 재라벨링부터 재학습까지 비용이 너무 커지기 때문이다. 
<br><br>
**클래스를 추가할 경우 주의사항**

ⓐ 시간 비용 낭비
인공지능 프로젝트에서 가장 시간과 돈이 많이 드는 병목 구간은 '데이터 라벨링'이다. 1차 프로젝트 때 혼자서 모든 YOLO학습을 도맡아서 진행했는데, 라벨링 작업만큼은 가장 귀찮고 하기 싫은 작업이었다. 그 불필요한 클래스를 위해 수천 장의 사진에 라벨링을 했던 시간이 아까울 수 있다. 하지만 학습결과가 어떻게 나올지 완벽하게 예상하기는 힘들기 때문에 본인 또한 무리하게 조난자 객체 탐지 프로젝트를 진행하면서 구명장비를 추가했다가 뺀 경험이 있다. 일부 사이트에서 자동 라벨링을 지원하긴 하지만 완벽하지는 않기 때문에 다시 검증해서 보완하는 작업이 필수적이다.  
<br><br>
ⓑ 추가 이미지 필요
라벨링도 라벨링이지만, 어떤 보조 프로그램이나 UI 테두리도 없는 가장 '순수한(Raw)' 이미지를 수백에서 수천 장 캡처하여 모으는 것도 쉬운 일은 아니다. 클래스를 추가하게 되면 데이터가 분산된다. 
<br><br>
ⓒ 데이터의 분산
그리고 또 경계해야할 부분은 . 총 1,000장의 사진이 있는데 클래스가 100개라면, 클래스당 평균 10장의 사진밖에 학습하지 못한다. 데이터의 양이 고정된 상태에서 클래스만 늘리면, 클래스당 학습량이 줄어들어 정확도가 급락하기 때문에 클래스를 늘리기 위해서는 충분한 데이터가 필수적이다. 
<br><br>
이러한 유의점들을 가슴 깊이 새긴 후에 클래스를 추가할지 말지를 선택하는 것은 철저한 분석과 개발자로서의 훌륭한 직감이 필요할 것이다. 

**한가지 주의점!**

그럼 **한 사진에 모든 클래스를 마구 마구 라벨링**해서 집어넣으면 되지 않을까라는 의문이 들 수 있다.

**1. 문맥적 편향 (Contextual Bias)의 발생**
인공지능은 우리가 생각하는 것보다 훨씬 눈치가 빠르고 게으르다. 
만약 데이터 결핍을 해결하려고 한 사진 안에 **[사람, 구명조끼, 튜브]** 를 항상 같이 넣어서 학습시킨다고 가정해 보자. 

모델은 '구명조끼' 자체의 형태를 열심히 외우는 대신, "아! 사람 옆에 주황색 덩어리가 있으면 대충 구명조끼구나!" 하고 꼼수를 부리며 `문맥(Context)`을 통째로 외워버린다.
결과적으로 나중에 바닥에 덩그러니 떨어져 있는 '구명조끼 단일 객체'를 보여주면, 주변에 사람이 없기 때문에 인식을 못 하는 `편향(Bias) 분산`이 발생한다.

**2. 가림 현상(Occlusion)과 해상도의 분산**

사진 한 장의 크기(해상도)는 고정되어 있다. (예: 640x480 픽셀)
이 한정된 도화지 안에 여러 클래스를 욱여넣으려면, 자연스럽게 객체들이 겹치거나 카메라에서 멀리 떨어져서 찍혀야 한다.

`가림(Occlusion)`: 객체들이 서로 겹치면서 특징이 절반씩 잘림.

해상도 저하: 객체 하나당 할당되는 픽셀 수가 급격히 줄어듦.

결국 모델은 각 클래스의 명확하고 뚜렷한 특징(Feature)을 온전히 학습하지 못하고, 찌그러지거나 잘린 조각들만 학습하게 되어 특징 데이터가 지저분하게 분산(노이즈)된다.

**3. 클래스 간 특징 간섭 (Feature Overlap)**
YOLO는 이미지를 `바둑판 모양의 그리드(Grid)`로 쪼개서 객체를 탐지한다.
한 사진 안에 여러 클래스의 객체가 너무 오밀조밀하게 붙어있으면, 모델이 "이 특징이 A 클래스 건가? B 클래스 건가?" 하고 헷갈리게 된다. 
즉, 모델이 정답을 찾기 위해 집중해야 할 가중치(Weight)가 여러 클래스 사이에서 분산되어 버린다. 

**요약하자면**,
한 장에 여러 클래스를 꽉꽉 채워 넣으면 **양적인 부족함(수량 분산)** 은 해결되겠지만, 겹치고 작아지고 배경과 섞이면서 **질적인 헷갈림(특징 분산)** 이 발생하게 된다. 혹 떼려다 혹 붙이는 격이다. 

따라서 가장 이상적인 데이터셋 구축 방법은 '여러 객체가 복잡하게 섞여 있는 실전형 사진'과 '단일 객체만 크고 선명하게 찍힌 정석형 사진'을 적절한 비율로 섞어주는게 좋다. 
...
위에서 설명했듯이 이렇게 낮은 형태의 F1곡선과 AP면적이 홀쭉한 클래스의 경우 학습이 잘 되지 않았다는 증거다. 그래서 눈물을 머금고 과감하게 클래스에서 제외시킨 후에 다시 학습을 재개했다. 학습효과가 1.5배 정도 향상되었다. mAP값은 아무래도 면적의 평균값이기 때문에 문제아를 제거만 해도 엄청나게 학습데이터가 향상된다. 물론 클래스가 수십개면 이야기가 달라지겠지만 말이다. 

---
<br><br> 
## 9. 시도해보고 싶었던 ignored

**ignored를 설정하는것이 마치 null -> "" 과 같다.** 
...
- **오탐(False Positive) 방지:** 모델이 "물체인데 라벨이 없는" 데이터를 보면 혼란을 겪는다. 확실하지 않은 물체나, 학습시키고 싶지 않은 잡음(Noise) 영역을 `ignored`로 설정하면 모델이 해당 영역을 정답이라고 오해하는 것을 방지할 수 있다.
