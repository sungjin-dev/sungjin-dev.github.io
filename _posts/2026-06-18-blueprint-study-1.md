...
<br><br>
그리고 `__name__`은 현재 파이썬 파일의 위치를 Flask에게 알려주는 값이다. Flask는 이를 이용해 템플릿이나 정적 파일(css, js)의 위치를 찾는다.
...
서버가 만들어지면, Flask는 요청된 URL을 확인한 뒤 등록된 라우팅 정보와 비교하여 알맞은 함수를 실행한다. 구체적으로 요청된 주소(예: `/login`)와 방식(GET, POST 등)을 보고 해당 로직(함수)으로 연결해주는 것!
...
<br><br>
그렇다고 `Blueprint`가 단순히 파일을 분리하는 기능만으로 쓰이진 않는다. 
<br>
회원관리, 게시판, 관리자 페이지처럼 독립적인 기능 단위를 하나의 묶음으로 관리할 수 있게 해준다.
<br>
덕분에 프로젝트 규모가 커져도 기능별로 유지보수가 쉬워진다.
