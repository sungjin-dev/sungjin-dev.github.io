...
한국은행(ECOS)과 통계청(KOSIS)의 OpenAPI를 연동하여 지역별 거시경제 데이터를 자동으로 수집하고 전처리하는 파이프라인 구축. 
GitHub Actions를 통해 매일 최신 데이터가 갱신되며, 아래 대시보드에서 결과를 확인 가능. 
...
## 프로젝트 목적
 ...
 한국은행 ECOS와 통계청 KOSIS의 공개 데이터를 이용해 지역별 경제 결정요인을 통계적으로 비교하고, 그 결과를 누구나 볼 수 있는 대시보드로 만드는 프로젝트
 -대전과 대구·울산의 차이점을 중심으로 기술-
...
γ 계수의 유의성으로 판단. 모델은 M1(실물) → M2(+금융) → M3(+대외·심리)로
단계 확장하며 조정 R²로 비교하여 결론 도출.

## 한계점

신용카드 사용액과 서비스업 생산은 동시에 움직이는 변수기 때문에 인과관계의 방향을 특정하기 어려움. (역인과관계의 오류) 

단순 OLS 다중회귀분석은 "X가 Y에 단방향으로 영향을 준다"고 가정하기 때문에, X와 Y가 서로 영향을 주고받는 동시성 문제가 있을 시 모델이 계산한 회귀계수(γ)가 부풀려지거나 왜곡되는 내생성(Endogeneity) 문제가 발생.

시차 모형(R1)으로 강건성만을 확인하고, 결론은 인과가 아닌 **연관성**으로 서술.
